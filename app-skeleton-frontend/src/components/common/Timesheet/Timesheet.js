...
import './Timesheet-white.css'

/**
 * Timesheet Bubble
 */
const Bubble = function (wMonth, min, start, end) {
  this.min = min
  this.start = start
  this.end = end
...
const TimeSheet = React.createClass({
  ...
  parse(data, min, max) {
    ...
    for (let n = 0, m = data.length; n < m; n++) {
      const beg = parseDate(data[n][0])
      const end = data[n].length === 4 ? parseDate(data[n][1]) : null
      const lbl = data[n].length === 4 ? data[n][2] : data[n][1]
      const cat = data[n][3] || 'default'
      ...
      ret.push({ start: beg, end, label: lbl, type: cat })
    }
  ...
  componentDidMount() {
    const widthMonth = ReactDOM.findDOMNode(this.refs.year).offsetWidth
    this.setState({
      widthMonth
    })
  },
  ...
  getLists(data, year) {
    const lists = []
    const widthMonth = this.state && this.state.widthMonth
    if (!widthMonth) return lists

    for (let i = 0, l = data.length; i < l; i++) {
      const cur = data[i]
      const bubble = new Bubble(widthMonth, year.min, cur.start, cur.end)
      ...
      const className = `bubble bubble-${cur.type || 'default'}`
      const duration = cur.end ? Math.round((cur.end - cur.start) / 1000 / 60 / 60 / 24 / 39) : ''
      const date = bubble.getDateLabel()
      const label = cur.label

      const line = [
        <span key="1" style={style} className={className} data-duration={duration} />,
        <span key="2" className="date">{date}</span>,
        <span key="3" className="label">{label}</span>
      ]
  ...
  render() {
    const result = this.parse(this.props.data, this.props.min, this.props.max)
    const data = result.data
    const year = result.year

    const sections = []
    for (let c = year.min; c <= year.max; c++) {
      sections.push(<section key={c} ref="year">{c}</section>)
    }

    const lists = this.getLists(data, year)
